...
list_forwards() {
    ...
    /^no_tcp[ \t]*=[ \t]*true/ { no_t = 1 }
    /^use_udp[ \t]*=[ \t]*true/ { use_u = 1 }
    /^\[\[endpoints\]\]/ {
        if (in_block == 1 && l != "" && r != "") {
            if (no_t == 1 && use_u == 1) net = "UDP"
            else if (no_t == 0 && use_u == 1) net = "TCP+UDP"
            else net = "TCP"
            printf "[%d] %s -> %s (%s)\n", count, l, r, net
            count++
        }
        in_block = 1
        l = ""; r = ""; net = "TCP"; no_t = 0; use_u = 0
    }
    END {
        if (in_block == 1 && l != "" && r != "") {
            if (no_t == 1 && use_u == 1) net = "UDP"
...
edit_forward() {
    ...
    curr_no_tcp=$(awk -v t="$num" 'BEGIN{c=0} /^\[\[endpoints\]\]/{c++} c==t && /^no_tcp[ \t]*=[ \t]*true/{print 1}' "$CONF_FILE" | head -n 1)
    curr_use_udp=$(awk -v t="$num" 'BEGIN{c=0} /^\[\[endpoints\]\]/{c++} c==t && /^use_udp[ \t]*=[ \t]*true/{print 1}' "$CONF_FILE" | head -n 1)
    
    [ -z "$curr_no_tcp" ] && curr_no_tcp=0
    [ -z "$curr_use_udp" ] && curr_use_udp=0
