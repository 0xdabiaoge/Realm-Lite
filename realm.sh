...
list_forwards() {
    ...
    echo -e "\n${YELLOW}--- 当前转发列表 ---${PLAIN}"
    # 使用 awk 解析配置文件，提取并格式化显示
    awk '
    BEGIN { count = 1; l = ""; r = ""; net = "TCP"; in_block = 0 }
    /^network[ \t]*=[ \t]*\{/ {
        # 解析 network 内联表写法
        no_tcp=($0 ~ /no_tcp[ \t]*=[ \t]*true/);
        use_udp=($0 ~ /use_udp[ \t]*=[ \t]*true/);
        if(no_tcp && use_udp) net="UDP";
        else if(!no_tcp && use_udp) net="TCP+UDP";
        else net="TCP";
    }
    /^\[\[endpoints\]\]/ {
        if (in_block == 1 && l != "" && r != "") {
...
add_forward() {
    ...
    net_str="network = { no_tcp = false, use_udp = false }"
    case "$proto_choice" in
        2) net_str="network = { no_tcp = true, use_udp = true }" ;;
        3) net_str="network = { no_tcp = false, use_udp = true }" ;;
    esac
...
edit_forward() {
    ...
    # 提取当前规则的内容用于默认展示
    curr_listen=$(awk -v t="$num" 'BEGIN{c=0} /^\[\[endpoints\]\]/{c++} c==t && /^listen/{split($0, a, "\""); print a[2]}' "$CONF_FILE" | head -n 1)
    curr_lport=$(echo "$curr_listen" | awk -F':' '{print $NF}')
    curr_raddr=$(awk -v t="$num" 'BEGIN{c=0} /^\[\[endpoints\]\]/{c++} c==t && /^remote/{split($0, a, "\""); print a[2]}' "$CONF_FILE" | head -n 1)
    ...
    curr_net_line=$(awk -v t="$num" '
    BEGIN { c=0 }
    /^\[\[endpoints\]\]/ { c++; next }
    /^network/ { if (c==t) print $0 }
    ' "$CONF_FILE" | head -n 1)
    
    curr_no_tcp=0; curr_use_udp=0
    echo "$curr_net_line" | grep -q "no_tcp[ \t]*=[ \t]*true" && curr_no_tcp=1
    echo "$curr_net_line" | grep -q "use_udp[ \t]*=[ \t]*true" && curr_use_udp=1
    ...
    new_net_str=""
    case "$proto_choice" in
        1) new_net_str="network = { no_tcp = false, use_udp = false }" ;;
        2) new_net_str="network = { no_tcp = true, use_udp = true }" ;;
        3) new_net_str="network = { no_tcp = false, use_udp = true }" ;;
        *) 
           if [ "$curr_net" = "UDP" ]; then
               new_net_str="network = { no_tcp = true, use_udp = true }"
           elif [ "$curr_net" = "TCP+UDP" ]; then
               new_net_str="network = { no_tcp = false, use_udp = true }"
           else
               new_net_str="network = { no_tcp = false, use_udp = false }"
           fi
           ;;
    esac
    ...
    # 使用 awk 替换对应序号的块
    tmp_file="/tmp/realm_config.toml.tmp"
    awk -v target="$num" -v nl="$new_lport" -v nr="$new_raddr" -v nnet="$new_net_str" '
    ...
    {
        if (in_target == 0) {
            # 过滤旧的 network 内联以及独立的 no_tcp/use_udp
            if ($0 !~ /^network[ \t]*=[ \t]*.*/ && $0 !~ /^[ \t]*no_tcp[ \t]*=[ \t]*.*/ && $0 !~ /^[ \t]*use_udp[ \t]*=[ \t]*.*/) {
                print $0;
            }
        }
    }
...
main_menu() {
    ...
    case "$num" in
        ...
        4) del_forward ;;
        5) edit_forward ;;
        6) list_forwards; echo ""; read -p "按回车键继续..." ;;
        7) view_log ;;
        8)
